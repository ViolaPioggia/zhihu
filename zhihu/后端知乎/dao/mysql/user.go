...
// var config = global.Config.Database.Mysql
// var db, _ = sql.Open("mysql", config.GetDsn())
var u model.User

func AddUser(username, password, nickname string) (bool, string) {
	EncryptPassword, err1 := utils.EncryptPassword(password) //加密密码
	if err1 != nil {
		return false, "encrypt password failed"
	}
	sqlStr := "insert into user(id,username,password,nickname,vip,create_time,update_time) values (?,?,?,?,?,?,?)"
	_, err := global.MysqlDB.Exec(sqlStr, FindID(), username, EncryptPassword, nickname, 0, time.Now(), time.Now())
	if err != nil {
...
func FindNickname(nickname string) bool {
	sqlStr := "select id,nickname from user where nickname=?"
	u.Nickname = nickname
	u.ID = 0
	// 非常重要：确保QueryRow之后调用Scan方法，否则持有的数据库链接不会被释放
